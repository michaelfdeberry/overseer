...
public abstract class PollingMachineProvider<TMachine> : MachineProvider<TMachine> where TMachine : Machine, IPollingMachine, new()
{
  ...
  protected virtual async Task Fetch(string resource, FetchRequest fetchRequest = default, CancellationToken cancellation = default)
  {
  ...
  protected virtual async Task<T> Fetch<T>(string resource, FetchRequest fetchRequest = default, CancellationToken cancellation = default)
  {
    using var client = new RestClient(new RestClientOptions
    {
      BaseUrl = new Uri(Machine.Url),
    ...
    if (!response.ContentType.Contains("json")) return default;
