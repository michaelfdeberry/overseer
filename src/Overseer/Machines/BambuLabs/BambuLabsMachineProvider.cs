...
namespace Overseer.Machines.BambuLabs;
...
public class BambuMachineProvider : MachineProvider<BambuMachine>
{
  ...
  Task OnMessage(MqttApplicationMessageReceivedEventArgs args)
  {
    ...
    status.Temperatures = new(){
      { 0 , new TemperatureStatus { HeaterIndex = 0, Actual = message.Print.BedTemperature, Target = message.Print.BedTargetTemperature } },
      { 1 , new TemperatureStatus { HeaterIndex = 1, Actual = message.Print.NozzleTemperature, Target = message.Print.NozzleTargetTemperature } },
    };
