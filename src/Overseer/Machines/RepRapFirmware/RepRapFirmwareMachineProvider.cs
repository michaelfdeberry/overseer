...
namespace Overseer.Machines.RepRapFirmware
{
  public class RepRapFirmwareMachineProvider : PollingMachineProvider<RepRapFirmwareMachine>
  {
    ...
    protected async Task<FetchRequest> PrepareRequest(FetchRequest request, CancellationToken cancellation = default)
    {
      ...
      (
        "rr_connect",
        new() { Query = new() { { "password", password }, { "sessionKey", "yes" } } },
        cancellation
      );
    ...
    protected override async Task<MachineStatus> AcquireStatus(CancellationToken cancellation)
    {
      ...
      if (status.State == MachineState.Operational || status.State == MachineState.Paused)
      {
        ...
        var fanIndex = activeTool.Fans.ElementAt(0);
    ...
    async Task<(int timeRemaining, double progress)> CalculateCompletion(ObjectModel model, IEnumerable<Extruder> extruders, CancellationToken cancellation)
    {
