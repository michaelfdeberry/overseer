...
abstract class AuthorizationDirective implements OnDestroy {
  ...
  get accessLevel(): AccessLevel {
    return this.accessLevelValue ?? AccessLevel.Readonly;
  }

  constructor(private authenticationService: AuthenticationService) {
    this.authChangeSubscription =
      this.authenticationService.authenticationChangeEvent$.subscribe(() =>
        this.update(),
      );
  }
  ...
  update(): void {
    ...
    if (!user.accessLevel) return this.accessDenied();
