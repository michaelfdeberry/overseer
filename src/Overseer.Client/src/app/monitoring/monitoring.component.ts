...
import {
  machineSortFunctionFactory,
  simpleMachineSort,
} from '../shared/machine-sorts';
...
export class MonitoringComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    forkJoin([
      ...
      this.machineStatusSubscription =
        this.monitoringService.statusEvent$.subscribe((status) => {
      ...
      this.mediaChangeSubscription = this.mediaObserver.media$.subscribe(
        (change: MediaChange) => {
  ...
  private resizeMachinesGrid() {
    ...
    if (['xs', 'sm'].indexOf(this.currentMediaSize) >= 0) {
      ...
      this.columns = Math.ceil(
        count / (Math.floor(count / base) + (count % base > 0 ? 1 : 0)),
      );
      this.rowHeight = 'fit';
    } else {
      this.columns = 3;
